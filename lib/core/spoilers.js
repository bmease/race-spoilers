...
export class SpoilerManager {
    ...
    async load() {
        ...
        logger.debug('dayRange: ', this.#races[0].dayRange)
    }
    ...
    getNextRace() {
        ...
        const afterSessionStart = race => race.sessions.start <= now
        const beforeSessionEnd = race => now <= race.sessions.end
        const currentSession = race => beforeSessionEnd(race) && afterSessionStart(race)
        
        const nextRace = race => now < race.sessions.start

        // return this.#races.find(race => (race.sessions.start <= now && now <= race.sessions.end) || (now < race.start))
        return this.#races.find(race => /* currentSession(race) ||  */nextRace(race))
                          .toObject()
    }
