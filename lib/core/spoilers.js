...
export class SpoilerManager {
    ...
    queryBlockedTime({ time }) {
        const race = this.#races.find(race => race.isDuringRace(time))
    ...
    getNextRace() {
        ...
        const afterRaceStart = race => race.range.start <= now
        const afterRaceEnd = race => now <= race.range.end
        const currentRace = race => afterRaceStart(race) && afterRaceEnd(race)

        const nextRace = race => now < race.range.start

        return this.#races.find(race => currentRace(race) ||  nextRace(race))
                          .toObject()
    }
    ...
    getPreviousRaces() {
        ...
        return this.#races.filter(race => race.blockedDayRange.end < now)
