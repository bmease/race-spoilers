...
export class Race {
     ...
     */
    get range() {
        const start = new Date(this.newSessions[this.firstSession])
        const end = new Date(this.newSessions[this.lastSession])
    ...
    get orderedSessions() {
        ...
        return Object.entries(this.newSessions).map(toDate)
                                               .sort(byTime)
    }
