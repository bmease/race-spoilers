...
const POPUP_FOOTER_SELECTOR = '.rs-footer'
    ...
    return `
        <table class="${NS}-session">
            <tr class="${NS}-session__row ${NS}-session__row--heading">
                <th class="${NS}-session__item">
                    ${browser.i18n.getMessage('popupTableSession')}
                </th>
                <th class="${NS}-session__item">
                    ${browser.i18n.getMessage('popupTableTime')}
                </th>
...
async function nextOrCurrentRaceEl() {
    ...
    return currentRace
        ? detailsEl(currentRace, browser.i18n.getMessage('popupHeadingNow'))
        : detailsEl(nextRace, browser.i18n.getMessage('popupHeadingNow'))
}
...
async function previousRacesEl() {
    ...
    return `
        <section class="${NS}-section">
            ${headerEl(browser.i18n.getMessage('popupHeadingPrevious'))}
            ${raceListEl(races)}
        </section>
    `
}
...
async function upcomingRacesEl() {
    ...
    return `
        <section class="${NS}-section">
            ${headerEl(browser.i18n.getMessage('popupHeadingUpcoming'))}
...
function popupFooterEl() {
    return `
        <a href="#" class="rs-options">
            ${browser.i18n.getMessage('popupLinkOptions')}
        </a>
    `
}
...
async function init() {
    ...
    const footerEl = document.querySelector(POPUP_FOOTER_SELECTOR)
    footerEl.innerHTML = popupFooterEl()

    footerEl.querySelector('.rs-options').addEventListener('click', optionsHandler)
